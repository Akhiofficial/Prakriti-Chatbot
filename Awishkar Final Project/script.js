const API_KEY = 'AIzaSyCkVMkeVs4GCg1rt5-A7EjMRZffMqmS54c';
...
recognition.onresult = function(event) {
    const userInput = event.results[0][0].transcript; // Get the recognized speech
    appendMessage(userInput, true); // Append user's voice input as a message
    handleUserInput(userInput); // Process the user's input
};
...
document.getElementById('voice-select').addEventListener('change', (event) => {
    selectedVoice = voiceMap.get(voices[event.target.value].name);
});
...
darkModeToggle.addEventListener('click', () => {
    ...
    // Optionally, you can change the icon when dark mode is toggled
    darkModeToggle.innerHTML = carousel.classList.contains('dark-mode') 
        ? '<i class="fas fa-sun"></i>' 
        : '<i class="fas fa-moon"></i>';
});

$(document).ready(function () {
    const videoCarouselInner = $('.video-carousel-inner');
    const videoItems = $('.video-item');

    // Clone video items for seamless looping
    videoItems.each(function () {
        videoCarouselInner.append($(this).clone());
    });

    // Set the animation duration based on the number of items
    const itemCount = videoItems.length;
    const animationDuration = 85; // Adjust this value to control speed

    // Set the total height to accommodate all items
    videoCarouselInner.css('height', `${itemCount * 100}%`); // Each item is 100% height

    // Set the animation duration
    videoCarouselInner.css('animation-duration', `${animationDuration}s`);
});